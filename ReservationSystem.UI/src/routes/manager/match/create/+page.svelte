<script lang="ts">
	...
	async function createMatch() {
		if (match.time < new Date()) {
			await Swal.fire('Invalid!', 'Match time cannot be in the past.', 'error');
			return;
		}
		if (match.homeTeam == match.awayTeam) {
			await Swal.fire('Invalid!', 'Home and away team cannot be the same.', 'error');
			return;
		}
		...
		if (response.ok) {
			...
			await Swal.fire('Failed!', 'Error creating match.', 'error');
			return;
		}
	}
</script>
