<script lang="ts">
	...
	async function createStadium() {
		if (stadium.rows <= 0) {
			await Swal.fire('Invalid!', 'Rows cannot be zero.', 'error');
			return;
		}
		if (stadium.columns <= 0) {
			await Swal.fire('Invalid!', 'Columns cannot be zero.', 'error');
			return;
		}
		...
		if (response.ok) {
			...
			await Swal.fire('Stadium Added!', 'Stadium has been added.', 'success');
			window.location.replace('/manager/stadium/create');
		} else {
			const error = await response.json();
			await Swal.fire('Failed!', 'Error adding stadium', 'success');
			return;
		}
	}
</script>
