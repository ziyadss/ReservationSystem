<script lang="ts">
	...
	async function getUsers() {
		...
		if (response.ok) {
			...
			await Swal.fire('Error!', 'HTTP-Error: ' + response.status, 'error');
		}
	}
	...
	async function deleteUser(username: string) {
		await Swal.fire({
			...
			if (result.value) {
				...
				if (response.ok) {
					await Swal.fire('User Deleted!', 'The user has been deleted.', 'success');
					await getUsers();
				} else {
					await Swal.fire('Failed!', 'HTTP-Error: ' + response.status, 'error');
				}
			}
		});
	}
	...
	async function authorizeUser(username: string) {
		await Swal.fire({
			...
			if (result.value) {
				...
				if (response.ok) {
					await Swal.fire('User Authorized!', 'The user has been authorized.', 'success');
					await getUsers();
				} else {
					await Swal.fire('Failed!', 'HTTP-Error: ' + response.status, 'error');
				}
...
<div class="container mt-5">
				...
				<tr>
					...
					<td>
						<button
							...
							on:click={() => authorizeUser(user.userName)}
							disabled={user.role.toLowerCase() == 'manager' || user.role.toLowerCase() == 'admin'}
							>Authorize</button
						>
					</td>
				</tr>
	...
	<div class="d-flex justify-content-between mt-3">
		<button class="btn btn-primary" on:click={previousPage} disabled={previous.length === 0}
			>Previous</button
		>
